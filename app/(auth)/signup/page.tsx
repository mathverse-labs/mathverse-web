...
const EXAM_GOALS = (["a-level-math1", "triam-udom", "mwit", "kvis", "general"] as ExamType[]);
...
export default function SignupPage() {
  ...
  const [examGoal, setExamGoal] = useState<ExamType | "">("");
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4 py-12">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="mt-4 text-2xl font-bold text-gray-900 font-sarabun">สมัครสมาชิก</h1>
          <p className="mt-1 text-sm text-gray-500 font-sarabun">เริ่มเตรียมสอบอย่างมีระบบ</p>
        </div>

        <div className="rounded-2xl border border-gray-100 bg-white p-8 shadow-sm space-y-4">
          {/* Google button */}
          <Button variant="outline" className="w-full font-sarabun gap-2" type="button">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-gray-700 font-sarabun">ชื่อ-นามสกุล</label>
              <input
                type="text"
                value={name}
                onChange={(e) => setName(e.target.value)}
                required
                placeholder="วรนาถ สุขสวัสดิ์"
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full rounded-lg border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-100 transition-all"
              />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-gray-700 font-sarabun">เป้าหมายการสอบ</label>
              <Select value={examGoal} onValueChange={(v) => setExamGoal(v as ExamType)}>
                <SelectTrigger className="w-full font-sarabun">
                  <span className="text-sm font-sarabun">
                    {examGoal ? EXAM_LABELS[examGoal] : "เลือกการสอบที่ต้องการ"}
                  </span>
                </SelectTrigger>
                <SelectContent>
                  {EXAM_GOALS.map((e) => (
                    <SelectItem key={e} value={e} className="font-sarabun">
                      {EXAM_LABELS[e]}
                    </SelectItem>
                  ))}
                </SelectContent>
